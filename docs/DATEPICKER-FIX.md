# Date Picker — Complete Replacement

## DO NOT redeploy couponmaxx. Push to main only. Test on Dr.Water.

**Problem:** The Polaris `<DatePicker>` component renders oversized inside a `<Popover>`. The calendar cells are too big, the whole thing takes up too much screen real estate, and it looks clunky compared to Alia's date picker.

**Fix:** Replace the ENTIRE `components/couponmaxx/DateRangePicker.tsx` file with the code below. This version:
- Shows just the button label normally (e.g. "Mar 20 – Mar 21")
- Clicking opens a COMPACT popover with presets only (Today, Yesterday, Last 7 days, etc.)
- Selecting "Custom range" shows a small inline calendar BELOW the presets (not side-by-side)
- The calendar is built with raw HTML/CSS, not Polaris DatePicker — so we control every pixel
- Selected range is highlighted, start/end dates are bolded
- Back + Apply buttons at the bottom
- Total popover width: ~280px. Compact.
...
export function DateRangePicker({ value, onChange }: Props) {
  ...
  const presetLabel = PRESETS.find(p => p.days === matched)?.label;
...
1. **No more Polaris `<DatePicker>`** — replaced with a custom `MiniCalendar` component that's 240px wide with 32px cells. Compact.
2. **Presets show as a vertical button list** — not Polaris OptionList (which has padding bloat). Each preset is a plain button with hover.
3. **Calendar shows BELOW presets** when "Custom range" is clicked — not side-by-side. This keeps the popover narrow (~280px).
4. **Two-click range selection** — first click sets start, second click sets end. A helper text says "Select start date" / "Select end date".
5. **Range highlighting** — start/end dates are blue, in-between dates have a light blue background. Today is bold. Future dates are greyed out and unclickable.
6. **Total popover width: 280px max.** Compare to the current 420px+.
...
# No Polaris DatePicker import:
grep "DatePicker" components/couponmaxx/DateRangePicker.tsx
# Should NOT show "import { ... DatePicker ... }" — only our MiniCalendar

# Compact width:
grep "minWidth" components/couponmaxx/DateRangePicker.tsx
# Should show 280 or similar, NOT 420 or 560

# MiniCalendar exists:
grep "MiniCalendar" components/couponmaxx/DateRangePicker.tsx
# Should exist
```
...
```bash
git add -A
git commit -m "fix: compact custom date picker, replace Polaris DatePicker with mini calendar"
git push
```

Wait 2 min. Open Dr.Water. Click the date picker. It should be tight, clean, and fast.
